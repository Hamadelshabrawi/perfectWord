...
function append_cozy_hover_effect_data_attributes( &$block_content, &$block ) {
	...
	if ( isset( $block['attrs']['cozyHoverEffect'] ) && in_array( $block['blockName'], $enabled_blocks, true ) ) {
		...
		$cozy_hover_string .= '--cozyDefaultBoxShadow: ' . $cozy_hover_effect['boxShadow']['horizontal'] . 'px ' . $cozy_hover_effect['boxShadow']['vertical'] . 'px ' . $cozy_hover_effect['boxShadow']['blur'] . 'px ' . $cozy_hover_effect['boxShadow']['spread'] . 'px ' . $cozy_hover_effect['boxShadow']['color'] . ' ' . $cozy_hover_effect['boxShadow']['position'] . '; --cozyHoverBoxShadow: ' . $cozy_hover_effect['boxShadowHover']['horizontal'] . 'px ' . $cozy_hover_effect['boxShadowHover']['vertical'] . 'px ' . $cozy_hover_effect['boxShadowHover']['blur'] . 'px ' . $cozy_hover_effect['boxShadowHover']['spread'] . 'px ' . $cozy_hover_effect['boxShadowHover']['color'] . ' ' . $cozy_hover_effect['boxShadowHover']['position'] . ';';

		preg_match( '/style="([^"]*)"/', $block_content, $matches );
		$existing_styles = $matches[1] ? $matches[1] : '';
...
function append_cozy_custom_font_data_attributes( &$block_content, &$block ) {
	...
	if ( isset( $block['attrs']['cozyCustomFont'] ) && in_array( $block['blockName'], $enabled_blocks, true ) ) {
		...
		if ( ! empty( $custom_font ) ) {
			...
			$block_content = '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=' . $custom_font . '" />' . $block_content;
